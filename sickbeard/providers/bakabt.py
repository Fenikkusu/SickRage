...
class BakabtProvider(generic.TorrentProvider):
    def __init__(self):

        generic.TorrentProvider.__init__(self, "BakaBT")
        ...
        self.cache = BakaCache(self)

        self.urls = {'base_url': 'http://www.bakabt.me/'}
...
class BakaCache(tvcache.TVCache):
    def __init__(self, provider):
        tvcache.TVCache.__init__(self, provider)

        # only poll BakabtTorrents every 15 minutes max
        self.minTime = 15
    ...
    def _getRSSData(self):
        ...
        logger.log(u"BakabtTorrents cache update URL: " + url, logger.DEBUG)

        return self.getRSSFeed(url)

provider = BakabtProvider()
